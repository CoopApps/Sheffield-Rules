...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("========================================");
    println!("EXPANDING NAME ABBREVIATIONS IN GENEALOGY");
    println!("========================================\n");

    // Get all records with names
    let records: Vec<(i64, String)> = sqlx::query_as(
        "SELECT id, name FROM unmatched_genealogy WHERE name IS NOT NULL AND name != ''"
    )
    .fetch_all(&pool)
    .await?;
    ...
    for (i, (id, original_name)) in records.iter().enumerate() {
        ...
        if &expanded_name != original_name {
            sqlx::query("UPDATE unmatched_genealogy SET name = ? WHERE id = ?")
                .bind(&expanded_name)
                .bind(id)
                .execute(&pool)
                .await?;
