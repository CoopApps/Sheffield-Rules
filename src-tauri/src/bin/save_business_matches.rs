...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("========================================");
    println!("SAVING BUSINESS MATCHES TO DATABASE");
    println!("========================================\n");

    // Create table for business matches
    println!("Creating genealogy_business_matches table...");
    sqlx::query(
        "CREATE TABLE IF NOT EXISTS genealogy_business_matches (
    ...
    println!("========================================");
    println!("FINDING AND SAVING MATCHES");
    println!("========================================\n");

    let mut high_confidence_saved = 0;
                    ...
                    if let Some(&count) = genealogy_name_counts.get(&norm_gen_name) {
                        if count == 1 {
                            ...
                            if profession_similarity(gen_prof_str, bus_occ_str) {
                                // Save high confidence match
                                sqlx::query(
                                    "INSERT INTO genealogy_business_matches
                                    (genealogy_id, business_id, genealogy_name, business_name,
                                     home_address, business_address, genealogy_profession,
                                     business_occupation, match_confidence)
                                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)"
                                )
                                .bind(gen_id)
                                .bind(bus_id)
                                ...
                                high_confidence_saved += 1;

                                if high_confidence_saved % 50 == 0 {
                                    println!("Saved {} matches...", high_confidence_saved);
                                }
                            }
                        }
                    }
    ...
    println!("SUMMARY");
    println!("========================================");
    println!("Successfully saved {} HIGH CONFIDENCE matches", high_confidence_saved);
    println!("These matches have:");
    println!("  ✓ Unique name (appears once in genealogy)");
    println!("  ✓ Different addresses (home vs work)");
    println!("  ✓ Matching profession");
    println!();
    println!("Data saved to: genealogy_business_matches table");
    println!("========================================\n");
