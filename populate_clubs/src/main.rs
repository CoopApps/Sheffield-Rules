...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Get all clubs
    let rows = sqlx::query("SELECT id, name FROM sheffield_clubs")
        .fetch_all(&pool)
        .await?;

    println!("Found {} clubs in database", rows.len());
    ...
    for (name_hint, division_id, position) in assignments {
        // Find club by name
        let club = rows.iter().find(|row| {
            let name: String = row.get("name");
            name.to_lowercase().contains(name_hint)
        });

        if let Some(club_row) = club {
            let club_id: String = club_row.get("id");
            let club_name: String = club_row.get("name");
            ...
            .bind(format!("{}-{}", club_id, division_id))
            .bind(division_id)
            .bind(&club_id)
            .bind(position)
            .bind(false)
            ...
            println!("✓ Assigned {} to {} at position {}", club_name, division_id, position);
            success += 1;
        } else {
