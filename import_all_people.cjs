...
const db = new sqlite3('Sheffield1867.db', { timeout: 10000 });
...
console.log('STEP 1: Import all people from census files');
console.log('='.repeat(60) + '\n');
...
for (const filename of csvFiles) {
    ...
    console.log(`Processing ${filename} (${records.length} people)...`);

    const transaction = db.transaction((recordList) => {
        let imported = 0;
        let linked = 0;
        ...
        for (const record of recordList) {
            ...
            // Check if this person matches an existing player
            const matchingPlayer = db.prepare(`
                SELECT id FROM sheffield_players
                WHERE name = ? AND birth_year = ?
                LIMIT 1
            `).get(name, birthYear);

            // Check if person already exists in sheffield_people
            ...
            `).run(
                personId, name, first, middle, surname,
    ...
    totalPeopleImported += result.imported;
    linkedToPlayers += result.linked;

    console.log(`  ✓ Imported ${result.imported} people (${result.linked} linked to players)`);
}

console.log('\n' + '='.repeat(60));
console.log(`Total people imported: ${totalPeopleImported}`);
console.log(`Linked to players: ${linkedToPlayers}`);
console.log(`Non-players: ${totalPeopleImported - linkedToPlayers}`);
